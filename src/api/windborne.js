...
const API_PATH = import.meta.env.VITE_API_PATH || `${BASE_URL}api/wb`;
...
async function fetchHour(i) {
  ...
  const url = `${API_PATH}/${file}`; // corrected variable name
  try {
    const { data } = await axios.get(url, { timeout: 8000 });
    return data;
  } catch (e) {
    // console.warn("hour fetch failed", url, e?.message);
    return null;
  }
}
